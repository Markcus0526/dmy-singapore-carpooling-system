...
- (void)handleTouchAtLocation:(CGPoint)touchLocation
{
	...
	for(int i = _starViews.count - 1; i >= 0; i--)
	{
		...
		if (touchLocation.x > imageView.frame.origin.x)
		{
			if (i == 0)
			{
				if (touchLocation.x < imageView.frame.origin.x + imageView.frame.size.width / 3)
					_rating = 0;
				else if (touchLocation.x < imageView.frame.origin.x + imageView.frame.size.width * 2 / 3)
					_rating = 0.5;
				else
					_rating = 1;
			}
